...
void OLED_Update(void)
{
    ...
    ssd1306_command(1);
#endif
    for(int i = 0; i < (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8);)
    {
        
        uint8_t control_ = 0x40; // Control byte to denote following bytes are data
        uint8_t data_to_send[17]; 
        data_to_send[0] = control_; // First byte is the control byte
        
        
        // Fill the rest of data_to_send with data bytes from buffer
        for (int x = 1; x <= 16 && i < (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8); x++, i++) {
            data_to_send[x] = buffer[i];
        }
        
        // Send the control byte followed by up to 16 data bytes in one I2C transaction
        I2C1_WriteNBytes(SSD1306_ADDR, data_to_send, sizeof(data_to_send));
    }
...
static void ssd1306_command(uint8_t command)
{
    uint8_t control_ = 0x00; // Control byte to denote following byte is a command
    uint8_t buffer[2] = {control_, command}; // Combine control byte and command into one buffer
   
    // Send both the control byte and the command byte in one I2C transaction
    I2C1_WriteNBytes(SSD1306_ADDR, buffer, sizeof(buffer));
}

static void ssd1306_data(uint8_t value)
{
    uint8_t control_ = 0x40;  
    uint8_t buffer[2] = {control_, value}; // Combine control byte and command into one buffer
